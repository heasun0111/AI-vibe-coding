...
- 모든 변경은 `main`에 직접 커밋·푸시하지 않고 PR로 진행한다(브랜치 명명, 커밋 규칙, 필수 GitHub
  저장소 설정은 `CONTRIBUTING.md` 참조).
- PR·`main` 병합 시 `.github/workflows/ci.yml`이 자동 실행되어 단위 테스트(Branch 커버리지 100%),
  순환복잡도·중복코드 게이트, 린트를 검증한다.

### 산출물 작성 공통 규칙

모든 A-SPICE 산출물(요구사항/아키텍처/상세설계/통합·시스템 테스트 등 `WP_Templates/` 기반 문서) 작성에
공통으로 적용한다.

- **표 없는 절도 표로 정리한다**: 공식 템플릿의 절(장)에 원래 표 형식이 없고 "작성 안내" 프로즈만 있는
  경우에도, 여러 항목·속성을 나열하는 내용이면 줄글 대신 적절한 컬럼의 표를 새로 만들어 그 안에
  작성한다. 사용자가 한눈에 보기 편한 구조(표 컬럼 구성, 정렬, 필요하면 소제목)를 항상 프로즈보다
  우선한다. 표로 구조화하기 어려운 서술적 내용(목적, 배경 설명 등)만 프로즈로 남긴다.
- **다이어그램은 원본과 렌더링 이미지를 분리한다**: Mermaid/PlantUML/drawio 등 다이어그램의 원본
  코드·파일은 문서 본문에 직접 넣지 않고 별도 파일로 저장한다(예: `<문서 파일명>_diagrams/<다이어그램
  ID>.mmd`/`.puml`/`.drawio`). 문서에는 그 원본을 렌더링한 이미지(PNG/SVG)만 삽입한다. 렌더링 도구가
  실제로 설치되어 있는지 먼저 확인하고, 없으면 추측하거나 원본 코드를 그대로 문서에 넣는 것으로
  대체하지 말고 "확인 필요 — 렌더링 도구 미설치"로 남겨 사용자에게 알린다.
